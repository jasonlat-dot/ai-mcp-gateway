...
import com.jasonlat.ai.domain.session.model.valobj.enums.SessionCustomKey;
    ...
    public Flux<ServerSentEvent<String>> handleGet(@PathVariable("gatewayId") String gatewayId,
        ...
        if (StringUtils.isBlank(gatewayId) || StringUtils.isBlank(sessionId)) {
            ...
            return Flux.just(ServerSentEvent.<String>builder()
                    ...
                    .event("error")
                    .data(JSON.toJSONString(Map.of("error", "Invalid or missing gatewayId/sessionId")))
                    .build());
        }
        ...
        try {
            if (StringUtils.isBlank(gatewayId) || StringUtils.isBlank(messageBody)) {
                ...
                return Mono.just(ResponseEntity
                        .badRequest()
                        .body(JSON.toJSONString(Map.of("error", "Invalid or missing gatewayId/messageBody")))
                );
            }
            ...
            return Mono.<ResponseEntity<?>>just(responseEntity)
                    .contextWrite(ctx -> ctx.put(SessionCustomKey.MCP_TRANSPORT_CONTEXT_KEY.getCode(), transportContext));
        } catch (Exception e) {
