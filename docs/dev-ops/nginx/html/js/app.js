...
$(document).ready(function() {
    ...
    $(document).on('click', '.btn-view-tools', function() {
        ...
        $.when(
            ...
            if(toolsResult && toolsResult.code === 'SUCCESS_0000' && toolsResult.data) {
                const toolsList = toolsResult.data;
                const protocolsList = (protocolsResult && protocolsResult.code === 'SUCCESS_0000' && protocolsResult.data) ? protocolsResult.data : [];
        ...
        reader.onload = function(event) {
            ...
            try {
                ...
                $.ajax({
                    ...
                    data: JSON.stringify(requestData),
                    success: function(response) {
                        if (response && response.code === 'SUCCESS_0000' && response.data && response.data.length > 0) {
